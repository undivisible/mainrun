//! Simple cosine learning rate experiment with actual training

use crate::{
    model::GPTConfig, 
    simple_training::{SimpleTrainer, SimpleTrainingConfig}, 
...
/// Simple cosine LR experiment
pub struct SimpleCosineLRExperiment {
    device: Device,
    config: SimpleCosineLRConfig,
}

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct SimpleCosineLRConfig {
...
impl Default for SimpleCosineLRConfig {
    fn default() -> Self {
        Self {
...
impl SimpleCosineLRExperiment {
    pub fn new(device: Device) -> Self {
        Self {
            device,
            config: SimpleCosineLRConfig::default(),
        }
    }
    
    pub fn with_config(mut self, config: SimpleCosineLRConfig) -> Self {
        self.config = config;
        self
    }
    
    /// Run the simple cosine LR experiment
    pub fn run(&self) -> anyhow::Result<ExperimentResult> {
        let start_time = Instant::now();
        
        println!("Starting SIMPLE cosine LR experiment");
        println!("Config: {:?}", self.config);
        
        // Load simple data
        let (data_loader, tokenizer) = tokio::task::block_in_place(|| {
            tokio::runtime::Handle::current().block_on(load_simple_data(
                self.config.base_config.vocab_size,
                10000, // num_titles
                0.1,   // val_frac
                self.device.clone(),
            ))
        ...
        let training_config = SimpleTrainingConfig {
            model_config: self.config.base_config.clone(),
            max_steps: self.config.max_steps,
            eval_interval: self.config.eval_interval,
            lr_scheduler: Box::new(SimpleCosineLrSchedulerWrapper {
        ...
        let mut trainer = SimpleTrainer::new(training_config, data_loader, tokenizer)?;
        
        // Run simple training
        println!("Beginning SIMPLE training with cosine LR scheduling...");
        ...
        let result = ExperimentResult {
            name: "simple_cosine_lr".to_string(),
            description: "SIMPLE cosine learning rate with warmup (0.025→0.001)".to_string(),
        ...
        println!("SIMPLE cosine LR experiment completed:");
    ...
    pub fn quick_test(&self) -> anyhow::Result<ExperimentResult> {
        ...
        println!("Running SIMPLE quick test ({} steps)...", test_exp.config.max_steps);
        test_exp.run()
    }
}

/// Wrapper for simple cosine LR to implement LrScheduler trait
struct SimpleCosineLrSchedulerWrapper {
    max_lr: f32,
    min_lr: f32,
    warmup_steps: usize,
    max_steps: usize,
...
impl LrScheduler for SimpleCosineLrSchedulerWrapper {
...
mod tests {
    ...
    #[test]
    fn test_simple_cosine_lr_schedule() {
        let scheduler = SimpleCosineLrSchedulerWrapper {
            max_lr: 0.025,
            min_lr: 0.001,
            warmup_steps: 100,
            max_steps: 896,
